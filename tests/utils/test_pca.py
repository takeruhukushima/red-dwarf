...
def test_run_reducer_toy():
    ...
    actual_projected_participants, _, pca = PcaUtils.run_reducer(
        vote_matrix=initial_matrix
    )
    actual_eigenvectors = pca.components_
    actual_eigenvalues = pca.explained_variance_
    assert_allclose(
        actual_projected_participants.values, expected_projections, rtol=10**-5
    )
...
def test_matching_explained_variance(polis_convo_data):
    ...
    _, _, actual_pca = PcaUtils.run_reducer(vote_matrix=real_vote_matrix)
...
@pytest.mark.parametrize(
    "polis_convo_data", ["small-with-mod-out-votes"], indirect=True
)
...
def test_run_reducer_real_data_below_100(polis_convo_data):
    fixture = polis_convo_data
    ...
    actual_projected_participants, actual_projected_statements, actual_pca = (
        PcaUtils.run_reducer(vote_matrix=real_vote_matrix)
    )
    ...
    expected_participant_projections = (
        helpers.transform_base_clusters_to_participant_coords(
    ...
    for expected in expected_participant_projections:
        ...
        assert_array_almost_equal(
            actual_projected_participants.loc[pid, ["x", "y"]].values, expected["xy"]
        )

    assert_array_almost_equal(
        actual_projected_statements.values.transpose(),
        expected_pca["comment-projection"],
    )


# TODO: Accomodate sign flips in "medium-no-meta".
@pytest.mark.parametrize("polis_convo_data", ["medium-with-meta"], indirect=True)
def test_run_reducer_real_data_above_100(polis_convo_data):
    fixture = polis_convo_data
    ...
    math_data = helpers.flip_signs_by_key(
        nested_dict=fixture.math_data, keys=["pca.comps[0]", "pca.center"]
    )
    expected_pca = math_data["pca"]
...
@pytest.mark.skip
def test_run_reducer_real_data_testing():
    ...
    loader = Loader(
        polis_id="r45ru4zfmutun54ttskne"
    )  # 336 voters, 15/148 meta NOPE 76/148 mismatch
